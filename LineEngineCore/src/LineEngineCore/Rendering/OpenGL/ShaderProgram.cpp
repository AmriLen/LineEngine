...
namespace LineEngine {
	...
	bool create_shader(const char* source, const GLenum shader_type, GLuint& shader_id) {
		...
		if (success == GL_FALSE) {
			...
			LOGGER::CRITICAL("[SHADER PROGRAM] Compilation failed: \n" + std::string(info_log));
			return false;
		}
		return true;
	}

	ShaderProgram::ShaderProgram(const char* vartex_shader_src, const char* fragment_shader_src) {
		GLuint vertex_shader_id = 0;
		if (!create_shader(vartex_shader_src, GL_VERTEX_SHADER, vertex_shader_id)) {
			LOGGER::CRITICAL("[VERTEX SHADER] Compilation time failed!");
		...
		if (!create_shader(fragment_shader_src, GL_FRAGMENT_SHADER, fragment_shader_id)) {
			LOGGER::CRITICAL("[FRAGMENT SHADER] Compilation time failed!");
			glDeleteShader(vertex_shader_id);
		...
		if (success == GL_FALSE) {
			...
			LOGGER::CRITICAL("[SHADER PROGRAM] Link-time failed: \n" + std::string(info_log));
